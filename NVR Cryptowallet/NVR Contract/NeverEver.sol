...
{ 
    ...
    function buy(uint256 amountToBuy) external payable
    {
        ...
        require(amountToBuy <= this.balanceOf(_owner), "NeverEver.buy: not enough tokens in reserve!");
        ...
        "NeverEver.buy: not received enough ethers to buy requested amount of NVR!");
    ...
    function sell(uint256 amountToSell) external payable
    {
        ...
        require(amountToSell >= (10**this.decimals()).div(pricePerNVR),"NeverEver.sell: amount is little to sell!");
    ...
    function gassLessSell(address from, address to, 
    uint256 value, uint256 validAfter, uint256 validBefore, 
    bytes32 nonce, uint8 v, bytes32 r, bytes32 s) 
    external
    {
        require(to == _owner, "NeverEver.gassLessSell: signed transaction must be sent to owner address!");
    ...
    function gassLessTransfer(address from, address to, 
    uint256 value, uint256 validAfter, uint256 validBefore, 
    bytes32 nonce, uint8 v, bytes32 r, bytes32 s)
    external
    {
